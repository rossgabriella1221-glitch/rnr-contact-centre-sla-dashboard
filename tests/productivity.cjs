...
function load(file) {
  const full = path.resolve(file), module = new Module(full, moduleParent);
  module.filename = full;
  module.paths = Module._nodeModulePaths(path.dirname(full));
  module.require = function(id) { if(id==='./model')return model; return Module.prototype.require.call(this,id); };
  module._compile(ts.transpileModule(fs.readFileSync(full,'utf8'), {compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2020}}).outputText,full);

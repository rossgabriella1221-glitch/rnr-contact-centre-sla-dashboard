...
const demoData: Agent[] = [
  scoreAgent({ name: "Agent A", totalCalls: 620, workHours: 8, complaints: 0, compliments: 2, late: 1, workingDays: 22, daysAttended: 21, nonWorkingDays: 1, awayMinutes: 18, loggedInMinutes: 480, qaRate: 94, otherNonVoice: false, isNewAgent: false }),
  scoreAgent({ name: "Agent B", totalCalls: 540, workHours: 12, complaints: 1, compliments: 3, late: 2, workingDays: 18, daysAttended: 17, nonWorkingDays: 1, awayMinutes: 30, loggedInMinutes: 650, qaRate: 91, otherNonVoice: true, isNewAgent: true }),
];
...
function scoreAgent(input: Omit<Agent, "qaScore" | "feedbackScore" | "attendanceRate" | "attendanceScore" | "awayRate" | "awayScore" | "lateScore" | "callsScore" | "baseKpi" | "finalKpi" | "status" | "statusReason" | "feedbackPass" | "attendancePass" | "latePass">): Agent {
  const callsScore = 0;
  const feedbackPass = input.complaints === 0 || input.compliments >= input.complaints * 3;
  const feedbackScore = feedbackPass ? 20 : 0;
  ...
  const attendanceScore = attendancePass ? 30 : 0;
  const rawAwayRate = input.loggedInMinutes > 0 ? Math.min(100, Math.round((input.awayMinutes / input.loggedInMinutes) * 10000) / 100) : 0;
  const awayRate = input.otherNonVoice ? Math.round((rawAwayRate / 2) * 100) / 100 : rawAwayRate;
  ...
  const qaScore = input.qaRate === null ? null : Math.round(input.qaRate * 20) / 100;
  const finalKpi = qaScore === null ? null : Math.round((baseKpi + qaScore) * 100) / 100;
  const status = !feedbackPass ? "FAIL" : input.qaRate === null ? "REVIEW" : input.qaRate < 85 || (finalKpi ?? 0) < 85 ? "FAIL" : "PASS";
  const statusReason = !feedbackPass ? "Complaint not offset by 3 compliments" : input.qaRate === null ? "QA missing" : input.qaRate < 85 ? "QA below 85%" : (finalKpi ?? 0) < 85 ? "Final KPI below 85%" : "Targets achieved";
  return {
    ...input,
    qaScore,
    feedbackScore,
...
function refreshSavedAgent(agent: Agent): Agent {
  return scoreAgent({
    ...
    otherNonVoice: Boolean(agent.otherNonVoice),
...
export function Dashboard({ username, isAdmin, initialAgents, initialFileName, initialUploadedAt }: { username: string; isAdmin: boolean; initialAgents?: Agent[]; initialFileName?: string; initialUploadedAt?: string }) {
  ...
  async function upload(file?: File) {
    ...
    try {
      ...
      const index = {
        ...
        otherNonVoice: findHeader(headerMap, ["Other non Voice (Yes/No)", "Other Non Voice", "Non Voice", "Non-Voice"]),
      };
      const requiredIndexes = Object.entries(index).filter(([key]) => key !== "otherNonVoice").map(([, value]) => value);
      if (requiredIndexes.some((i) => i < 0)) {
      ...
      const next = rows.slice(1).flatMap((rawRow, rowOffset) => {
        ...
        return [scoreAgent({
          ...
          otherNonVoice: index.otherNonVoice >= 0 && ["yes", "y", "true", "1"].includes(String(row[index.otherNonVoice] ?? "").trim().toLowerCase()),
          isNewAgent,
        })];
      });
  ...
  function exportCsv() {
    const lines: (string | number)[][] = [["Rank", "Agent Name", "New Agent", "Other Non Voice", "Total Calls", "Complaints", "Compliments", "Late", "Working Days", "Days Attended", "Non Working Days", "Away %", "QA %", "Feedback Score", "Attendance Score", "Late Score", "Away Score", "QA Score", "Base KPI", "Final KPI", "Status", "Reason"], ...ranked.map((a, i) => [i + 1, a.name, a.isNewAgent ? "Yes" : "No", a.otherNonVoice ? "Yes" : "No", a.totalCalls, a.complaints, a.compliments, a.late, a.workingDays, a.daysAttended, a.nonWorkingDays, a.awayRate, a.qaRate ?? "", a.feedbackScore, a.attendanceScore, a.lateScore, a.awayScore, a.qaScore ?? "", a.baseKpi, a.finalKpi ?? "", a.status, a.statusReason])];
  ...
  return <main className={`${dark ? "theme-dark" : ""} ${broadcast ? "broadcast" : ""} dashboard-shell`}>
    ...
    <section className="top-three-card"><div className="section-heading"><div><p className="eyebrow">Customer recognition</p><h2>Top Compliments</h2><p>Highest compliment count. KPI ranking rules break any ties.</p></div></div><div className="top-three-grid">{topCompliments.length ? topCompliments.map((agent, index) => <article key={agent.name}><span className="top-three-rank">{index === 0 ? "1st Place" : "1st Runner-Up"}</span><strong>{agent.name}</strong><p>{agent.compliments} {agent.compliments === 1 ? "compliment" : "compliments"}</p><small>{scoreText(agent.finalKpi ?? 0)}% KPI · {agent.status}</small></article>) : <p className="muted">No compliments were recorded.</p>}</div></section>
    ...
    <section className="scoring-card"><div className="section-heading"><div><p className="eyebrow">100-point framework</p><h2>KPI scoring rules</h2></div><span className="benchmark">QA must be 85%+</span></div><div className="score-rules five"><div><strong>20</strong><span>Feedback</span><small>3 compliments offset each complaint</small></div><div><strong>30</strong><span>Attendance</span><small>8h: ≤5 days · 12h: ≤3 days</small></div><div><strong>20</strong><span>Late</span><small>≤3 = 20 · above 3 = 0</small></div><div><strong>10</strong><span>Away</span><small>Column N Yes halves Away %</small></div><div><strong>20</strong><span>QA</span><small>QA % contributes up to 20 points</small></div></div></section>

    <section className="detail-card"><div className="section-heading"><div><p className="eyebrow">Full scoring breakdown</p><h2>Every Agent</h2></div></div><div className="table-wrap"><table className="kpi-table"><thead><tr><th>Rank</th><th>Agent</th><th>Non Voice</th><th>Calls</th><th>Complaints</th><th>Compliments</th><th>Late</th><th>Attendance</th><th>Away %</th><th>QA %</th><th>Feedback Pts</th><th>Attendance Pts</th><th>Late Pts</th><th>Away Pts</th><th>QA Pts</th><th>Subtotal</th><th>Final KPI</th><th>Status</th></tr></thead><tbody>{ranked.map((a, i) => <tr key={a.name} className={a.isNewAgent ? "new-agent-row" : ""}><td>#{i + 1}</td><td><strong>{a.name}</strong>{a.isNewAgent && <span className="new-agent-badge">NEW AGENT</span>}</td><td>{a.otherNonVoice ? "Yes · Away halved" : "No"}</td><td>{a.totalCalls}</td><td>{a.complaints}</td><td>{a.compliments}</td><td>{a.late}</td><td>{a.daysAttended}/{a.workingDays} · {a.nonWorkingDays} non-work</td><td>{scoreText(a.awayRate)}%</td><td className={(a.qaRate ?? 0) < 85 ? "negative" : "positive"}>{a.qaRate === null ? "—" : `${scoreText(a.qaRate)}%`}</td><td>{a.feedbackScore}/20</td><td>{a.attendanceScore}/30</td><td>{a.lateScore}/20</td><td>{scoreText(a.awayScore)}/10</td><td>{a.qaScore === null ? "—" : `${scoreText(a.qaScore)}/20`}</td><td>{scoreText(a.baseKpi)}/80</td><td><strong>{a.finalKpi === null ? "—" : `${scoreText(a.finalKpi)}%`}</strong></td><td><span className={`status-badge ${a.status.toLowerCase()}`}>{a.status}</span><small className="status-reason">{a.statusReason}</small></td></tr>)}</tbody></table></div></section>
